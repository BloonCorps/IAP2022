...
#         booli = all(xi > [x1_min, x2_min]) and all(xi < [x1_max, x2_max])
#         if not booli:
#             accept_bool[i] = False
...
# with open('../output/TREMC/samples_beta_%.3f.pkl'%beta, 'wb') as file_handle:
...
for i in range(num_reps):
    ...
    plt.tight_layout()
    plt.savefig("../output/TREMC/%.3f.pdf"%beta_lst[i])
