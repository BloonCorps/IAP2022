...
for step in tqdm(range(nsteps), bar_format='{l_bar}{bar:15}{r_bar}{bar:-10b}'):
    ...
    x1_min_1, x1_max_1 = x1_min, 0.0
    x2_min_1, x2_max_1 = 0.55, x2_max
    cutoff = -0.3
    for i, xi in enumerate(x_candidate):
        booli = all(xi > [x1_min_1, x2_min_1]) and all(xi < [x1_max_1, x2_max_1]) and xi[0]*xi[1] < cutoff
        if not booli:
            accept_bool[i] = False
...
with open('../output/TREMC_basin1/samples_beta_%.3f.pkl'%beta, 'wb') as file_handle:
...
for i in range(num_reps):
    ...
    plt.tight_layout()
    plt.savefig("../output/TREMC_basin1/%.3f.pdf"%beta_lst[i])
