...
masks += [("Scale", [0.0, 1.0], x1_min, x1_max),("Scale", [1.0, 0.0], x2_min, x2_max)]

## dimenstion of hidden units used in scale and translation transformation
hidden_dim = 16
with open('../output/LBAR/nn_architect_beta%.3f.pkl', 'wb') as file_handle:
...
ubx = np.matmul(compute_cubic_spline_basis(xb_train.numpy()), theta2)
...
#         print('idx_step: %d ....\n'%idx_step)
    ...
    loss = torch.mean(phiF)+torch.mean(phiR)
    optimizer.zero_grad()
    loss.backward()
    optimizer.step()
    ...
    if idx_step in num_steps:
        with torch.no_grad():
            m_xa, logdetm = realNVP(torch.from_numpy(xa_valid))
            ubmx = torch.tensor(np.matmul(compute_cubic_spline_basis(m_xa.numpy()), theta2))
            phiF = ubmx - uax_valid - beta**(-1)*logdetm
            ...
            phiR = uamm1x - ubx_valid - beta**(-1)*logdetmm1

            loss_valid = torch.mean(phiF) + torch.mean(phiR)
        ...
        torch.save({'loss_train': loss.item(),
                    ...
                    '../output/LBAR/model_beta%.3f_step%d.pt'%(beta, idx_step))

## store training and validation loss
with open('../output/LBAR/loss_record_beta%.3f.pkl'%beta, 'wb') as file_handle:
