...
class RealNVP(nn.Module):
    def __init__(self, masks, hidden_dim):
        ...
        for i in range(len(masks)):
            ...
            elif masks[i][0] == 'Scale_inverse':
                self.transforms.append(Scale_inverse(self.masks[i], masks[i][2], masks[i][3]))
...
class Scale_inverse(nn.Module):
    def __init__(self, mask, low, high):
        super(Scale_inverse, self).__init__()
        self.mask = nn.Parameter(torch.Tensor(mask), requires_grad = False)
        self.low = nn.Parameter(torch.Tensor([low]), requires_grad = False)
        self.high = nn.Parameter(torch.Tensor([high]), requires_grad = False)
    ...
    def forward(self, x):
        ## forward is the inverse of Scale()
        y = (1-self.mask)*(x-self.low)/(self.high-self.low) + self.mask*0.5
        y = self.mask*x + (1-self.mask)*torch.log(y/(1-y))

        p = (1-self.mask)*(x-self.low)/(self.high-self.low) + self.mask*0.5
        logdet = torch.sum((1-self.mask)*(-torch.log(p)-torch.log(1-p)-torch.log(self.high - self.low)), -1)        
        return y, logdet

    def inverse(self, y):
        ## inverse here is the forward of Scale()
        x = self.mask*y + (1-self.mask)*(self.low + (self.high - self.low)*torch.sigmoid(y))
        y = (1-self.mask)*y
        logdet = torch.sum((1-self.mask)*(functional.logsigmoid(y) + functional.logsigmoid(-y) + torch.log(self.high - self.low)), -1)
        return x, logdet
