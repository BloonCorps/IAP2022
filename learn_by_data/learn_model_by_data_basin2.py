...
with open("../output/TREMC_basin2/samples_beta_{:.3f}.pkl".format(beta), 'rb') as file_handle:
    data = pickle.load(file_handle)
...
for idx_step in tqdm(range(num_saved_steps[-1] + 1)):
    ...
    if idx_step in num_saved_steps:
        ...
        torch.save({'masks': masks,
                    ...
                    "../output/model_trained_with_data_basin2/model_beta_{:.3f}_step_{}.pt".format(beta, idx_step))
...
fig.savefig("../output/loss_model_trained_with_data_basin2_beta_{:.3f}.pdf".format(beta))
